...
async def _send_evening_missed_check() -> dict[str, int]:
    """Создать зависимости и выполнить вечернюю рассылку."""
    bot = Bot(token=settings.TELEGRAM_BOT_TOKEN)
    try:
        service = ReminderMailingService(sender=TelegramSender(bot))
        return await service.send_evening_missed_check()
    finally:
        await bot.session.close()
...
@shared_task(name="reminder.tasks.send_evening_missed_check",
             ignore_result=True)
def send_evening_missed_check() -> dict[str, int]:
    """Запустить вечерний вопрос о незакрытых задачах."""
    result = async_to_sync(_send_evening_missed_check)()
    logger.info(
        "Evening missed check finished | processed=%s sent=%s failed=%s "
        "skipped=%s",
        result["processed"],
        result["sent"],
        result["failed"],
